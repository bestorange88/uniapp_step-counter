					    ...
					    html += '<div class="card card-aside">';
					        ...
					        html += '<div class="card-body">';
					            ...
					            if (item.catalog == 1) {
						            ...
						            if (item.power > 0) {
						            	html += '<div class="card-subtitle">活力值：' + number_format(item.power) + '</div>';
						            }
					            } else if (item.catalog == 2) {
									html += '<div class="card-subtitle">活力值：' + number_format(item.power) + '</div>';
					            }
					            if (item.catalog == 1) {
					            	html += '<a href="javascript:void(0)" class="btn btn-primary btn-sm btn-choose" data-currency="' + unit + '" data-id="' + item.id + '"><i class="fe fe-shopping-cart"></i> ' + number_format(item.price) + ' 乐豆</a>';
					            } else {
